...
const codeFenceDiffChunks: MergedChunk[] = [
  {
    ...
    lines: [
      ...
      {
        type: 'delete',
        content: 'npx skills add yoshiko-pg/diffops # エージェントにスキルを追加',
        oldLineNumber: 4,
        newLineNumber: undefined,
      },
      {
        type: 'add',
        content: 'npx skills add yoshiko-pg/diffops # エージェントにスキル群を追加',
...
describe('MarkdownDiffViewer', () => {
  ...
  it('renders changed fenced code blocks without dropping surrounding markdown in Diff Preview', () => {
    ...
    const newLine = screen.getByText(
      'npx skills add yoshiko-pg/diffops # エージェントにスキル群を追加',
    );
